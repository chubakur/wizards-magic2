...
# Внимание!! Для того, чтоsбы слои не наслаивались,
# я использую объект surface_backup , который является копией изображения.
...
class Connect(threading.Thread):
            ...
            if do_read:
                try:
                    ...
                    while len(answ) < MSGLEN:
                        answ += self.sock.recv(MSGLEN - len(answ))
                except ValueError:
    ...
    def __init__(self, sock, addr):
        ...
        #    self.sock.send(
        #        json.dumps(
        #            {"answ":100, "players":len(connections[self.game_id])}
        #        )
        #    )
        #    return
        # self.game = False
        ...
        while wzglobals.running:
            ...
            if query['action'] == "join":
                ...
                if len(wzglobals.players[self.game_id]) == 0:
                    wzglobals.games_cards[self.game_id]['water'] = \
                        cards.water_cards_deck[:]
                    wzglobals.games_cards[self.game_id]['fire'] = \
                        cards.fire_cards_deck[:]
                    wzglobals.games_cards[self.game_id]['air'] = \
                        cards.air_cards_deck[:]
                    wzglobals.games_cards[self.game_id]['earth'] = \
                        cards.earth_cards_deck[:]
                    wzglobals.games_cards[self.game_id]['life'] = \
                        cards.life_cards_deck[:]
                    wzglobals.games_cards[self.game_id]['death'] = \
                        cards.death_cards_deck[:]
                ...
                wzglobals.players[self.game_id][self.id].id = len(
                    wzglobals.players[self.game_id]
                )
                ...
                self.send(
                    self.sock,
                    {
                        "answ": 200,
                        "action": "join",
                        "id": wzglobals.players[self.game_id][self.id].id
                    }
                )
                # if num_players == 2:
                #    # когда заходит второй игрок. раздаем карты и ману.
                if (
                    len(wzglobals.players[self.game_id]) == 2 and
...
#                    for gamer in wzglobals.players[self.game_id]:
#                        # GAMER = PLAYER. просто почему-то питон неверно
#                        # обрабатывает в таком случае сообщения
#                        gamer.get_mana()
#                        gamer.get_cards()
                    deck_cards = []
                    deck_cards.append(
                    ...
                    for connection in connections[self.game_id]:
                        ...
                        self.send(
                            connection.sock,
                            {
                                "answ": 200,
                                "nicknames": [
                                    connections[self.game_id][0].nickname,
                                    connections[self.game_id][1].nickname
                                ],
                                "action": "update",
                                "mana": [
                                    wzglobals.players[self.game_id][0]
                ...
                try:
                    # send to client signal that server is going down
                    self.send(
                        self.sock,
                        {"answ": 200, "action": "server_close"}
                    )
                except:
                    pass
                self.sock.close()
