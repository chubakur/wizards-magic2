...
class Point(pygame.sprite.Sprite):
    ...
    def __init__(self):
        ...
        self.image = pygame.image.load(
            current_folder+'/misc/point_alpha.gif'
        ).convert_alpha()
        ...
        if event.type == pygame.KEYUP:
            if event.key == pygame.K_ESCAPE:
                if not wzglobals.question:
                    if 'importantmessage' not in wzglobals.__dict__:
                        ...
                        wzglobals.information_group.remove(
            ...
            if (
                ...
                wzglobals.answer = \
                    wzglobals.answer + pygame.key.name(event.key)
            if (
                wzglobals.question and (
                    event.key == K_RETURN or
                    len(wzglobals.answer) >= wzglobals.answer_maxchar
                )
            ):
            ...
            if event.button == 1:
                ...
                if wzglobals.stage == 0:
                    collided = pygame.sprite.spritecollide(
                        wzglobals.point, wzglobals.menu_group, 0
                    )
                elif wzglobals.stage <= 2:
                    collided = pygame.sprite.spritecollide(
                        wzglobals.point, wzglobals.cards_in_deck, 0
                    )
                    if not collided:
                        collided = pygame.sprite.spritecollide(
                            wzglobals.point, wzglobals.interface, 0
                        )
                    if not collided:
                        collided = pygame.sprite.spritecollide(
                            wzglobals.point, wzglobals.panels, 0
                        )
                if not collided:
                    return
                ...
                if (
                    item.type == 'button' or
                    item.type == 'txtinput' or
                    item.type == 'checkbox'
                ):
                    item.onmousedown()
                    return
                if item.type == "warrior_card":
                    # Карта в колоде! Карта на поле в cardbox
                    # exec(
                    #   'selected_card_0 = cards.' +
                    #   item.name +
                    #   '()'
                    # )  # Переменной selected_card_0 присваиваем новый объект
                    wzglobals.selected_card = cards.links_to_cards[item.name]()
                    # из локальной в глобальную
                ...
                if item.type == "magic_card":  # карта магии в колоде
                    ...
                    #   item.name +
                    #   '()'
                    # ) #в переменную selected_card засовываем одну такую карту
                    # exec(
                    #   'available_mana = wzglobals.player.' +
                    #   selected_card.element +
                    #   '_mana'
                    # ) # Вычисляем сколько маны у нас есть.
                    # # Значение помещаем в локальную переменную available_mana
                    available_mana = \
                        wzglobals.player.mana[selected_card.element]
                    if available_mana >= selected_card.level:
                        # exec(
                        #   'wzglobals.player.' +
                        #   selected_card.element +
                        #   '_mana -= ' +
                        #   str(selected_card.level)
                        # ) #Отнимаем ману
                        wzglobals.player.mana[selected_card.element] -= \
                            selected_card.level
                        wzglobals.player.action_points = \
                            False  # ставим запись, что ход сделан
                        ...
                        selected_card.cast()
                        # вызываем магию, периодизация делается уже внутри
                        # класса, путем добавления в группу
                        # wzglobals.magic_cards
                ...
                if wzglobals.cast_focus:  # выбор цели для каста
                    if item.type == 'cardbox':
                        wzglobals.cast_focus_wizard.focus_cast_action(
                            item.card
                        )
                        sockets.query(
                            {
                                "action": "cast",
                                "position": wzglobals.cast_focus_wizard
                                .parent.position,
                ...
                if wzglobals.cli:
                    if item.player.id != wzglobals.player_id:
                        return
                if item.type == "cardbox":
                    # Если клик на карточный бокс
                    if item.card.name != "player":
                        # Если в этом блоке есть карта
                        if item.card.cast:  # если есть каст
                            if not item.card.used_cast:
                                # если еще не кастовали
                    ...
                    if wzglobals.selected_card:  # если выбрана карта
                        if item.card.name != 'player':
                            ...
                            return
                        # если уже ходил
                        if not wzglobals.player.action_points:
                            wzglobals.gameinformationpanel.display(
                                "You've already made a move."
                            )
                            return
                        ...
                        # exec(
                        #   'available_mana = wzglobals.player.' +
                        #   wzglobals.selected_card.element +
                        #   '_mana'
                        # ) # Вычисляем сколько маны у нас есть.
                        # # Значение помещаем в локальную переменную
                        # # available_mana
                        available_mana = wzglobals.player.mana[
                            wzglobals.selected_card.element
                        ]
                        ...
                        # функция которая хранит описание действий при выводе
                        # карты
                        item.card.summon()
                        ...
                        #   wzglobals.selected_card.element +
                        #   '_mana -= ' +
                        #   str(wzglobals.selected_card.level)
                        # )  # Отнимаем ману
                        wzglobals.player.mana[
                            wzglobals.selected_card.element
                        ] -= wzglobals.selected_card.level
                        # # очищаем группу карты в колоде
                ...
                if not collided:
                    return
                item = collided[len(collided)-1]
                if item.type == "warrior_card" or item.type == "magic_card":
                    # по боевой карте
